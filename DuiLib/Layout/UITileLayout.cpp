...
namespace DuiLib
{
	...
	int CTileLayoutUI::GetChildVPadding() const
	{
        ...
        return GetScaleInt(m_iChildVPadding, m_nScale);
	}
	...
	{
        return GetScaleSize(m_szItem, m_nScale);
    }
	...
	void CTileLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		if (m_nColumnsFixed == 0) { 
			...
			cyNeeded = m_nRows*m_szItem.cy + (m_nRows-1)* iChildPadding;
		}
		else {
		...
		for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) {
			...
			int iPosY = rc.top + iColumnIndex*(m_szItem.cy+iChildPadding);
			if( m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible() ) {
