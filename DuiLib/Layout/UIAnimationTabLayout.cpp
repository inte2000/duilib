...
namespace DuiLib {
	...
	{
		return DUI_CTR_ANIMATETABLAYOUT;
	}

	LPVOID CAnimationTabLayoutUI::GetInterface(LPCTSTR pstrName)
	{
		if (_tcscmp(pstrName, DUI_CTR_ANIMATETABLAYOUT) == 0) return static_cast<CAnimationTabLayoutUI*>(this);
		return CTabLayoutUI::GetInterface(pstrName);
	}

	bool CAnimationTabLayoutUI::SelectItem(int iIndex, bool bTriggerEvent)
	{
	...
	void CAnimationTabLayoutUI::OnTimer( UINT_PTR nTimerID )
	{
