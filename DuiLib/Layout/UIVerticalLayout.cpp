...
namespace DuiLib
{
	...
	void CVerticalLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) {
			...
			if(!pControl->IsUsingFixedHeight()) {
				nAdjustables++;
	...
	{
        return GetScaleInt(m_iSepHeight, m_nScale);
    }
		...
		if( m_iSepHeight != 0 ) {
			...
			if( event.Type == UIEVENT_MOUSEMOVE )
			{
				if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
                    ...
                    m_cxyFixed.cy = GetScaleInt(m_rcNewPos.bottom - m_rcNewPos.top, m_nScale);
					if( m_bImmMode ) {
						m_rcItem = m_rcNewPos;
						NeedParentUpdate();
					}
	...
	{
		int iSepHeight = GetSepHeight();
		if( (m_uButtonState & UISTATE_CAPTURED) != 0 && bUseNew) {
			if(iSepHeight >= 0 )
				return CDuiRect(m_rcNewPos.left, MAX(m_rcNewPos.bottom - iSepHeight, m_rcNewPos.top),
				m_rcNewPos.right, m_rcNewPos.bottom);
			else 
				return CDuiRect(m_rcNewPos.left, m_rcNewPos.top, m_rcNewPos.right, 
				MIN(m_rcNewPos.top - iSepHeight, m_rcNewPos.bottom));
		}
		else {
			if(iSepHeight >= 0 )
				return CDuiRect(m_rcItem.left, MAX(m_rcItem.bottom - iSepHeight, m_rcItem.top), m_rcItem.right,
				m_rcItem.bottom);
			else 
				return CDuiRect(m_rcItem.left, m_rcItem.top, m_rcItem.right, 
				MIN(m_rcItem.top - iSepHeight, m_rcItem.bottom));
