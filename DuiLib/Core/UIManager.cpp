...
tagTDrawImgList::tagTDrawImgList()
{
    Clear();
}

tagTDrawImgList::tagTDrawImgList(LPCTSTR lpsz)
{
    Clear();
    sDrawString = lpsz;
}

void tagTDrawImgList::tagTDrawImgList::Clear()
{
    sDrawString.Empty();
    sImageName.Empty();
    bLoaded = false;
    ...
    count = 0;
    bSupportScale = false;
    ::SetRectEmpty(&rcBmpPart);
}

TDrawInfo tagTDrawImgList::GetDrawInfo(int index)
{
    TDrawInfo di;

    di.sDrawString = sDrawString;
    di.sImageName = sImageName;
    di.bLoaded = true;
    di.pImageInfo = pImageInfo;
    di.bSupportScale = bSupportScale;
    di.bHole = false;
    di.uFade = 255;
    ...
    int sw = (rcBmpPart.right - rcBmpPart.left) / count;
    di.rcBmpPart.left = rcBmpPart.left + index * sw;
    di.rcBmpPart.right = di.rcBmpPart.left + sw;
    di.rcBmpPart.top = rcBmpPart.top;
    di.rcBmpPart.bottom = rcBmpPart.bottom;

    return di;
}

TDrawInfo tagTDrawImgList::GetLastDrawInfo()
{
    ASSERT(count > 0);
    return GetDrawInfo(count - 1);
}
...
bool CPaintManagerUI::IsResourceFileExisted(LPCTSTR filename)
{
    bool bExisted = false;
    CDuiString pathfile = CPaintManagerUI::GetResourcePath();
    if (CPaintManagerUI::GetResourceZip().IsEmpty()) {
        pathfile += filename;
        bExisted = ::PathFileExists(pathfile) ? true : false;
    }
    else {
        pathfile += CPaintManagerUI::GetResourceZip();
        HZIP hz = NULL;
        if (CPaintManagerUI::IsCachedResourceZip()) hz = (HZIP)CPaintManagerUI::GetResourceZipHandle();
        else hz = OpenZip((void*)pathfile.GetData(), 0, 2, CPaintManagerUI::GetResourceZipPassword());
        if (hz != NULL) {
            ZIPENTRY ze;
            int i;
            bExisted = (FindZipItem(hz, filename, true, &i, &ze) == 0) ? true : false;
...
void CPaintManagerUI::SetResourceZip(LPVOID pVoid, unsigned int len, const char *pStrPwd)
{
    int nLen = 0;
    if (pStrPwd != nullptr)
        nLen = strlen(pStrPwd);
    ...
    if(nLen > 0)
    {
        strncpy_s(m_szResourceZipPassword, 64, pStrPwd, nLen);
        m_szResourceZipPassword[63] = 0; //确保截断
    }
    else
    ...
    if( m_bCachedResourceZip ) 
        m_hResourceZip = (HANDLE)OpenZip(pVoid, len, 3, CPaintManagerUI::GetResourceZipPassword());
}

void CPaintManagerUI::SetResourceZip(LPCTSTR pStrPath, bool bCachedResourceZip, const char *pStrPwd)
{
    int nLen = 0;
    if (pStrPwd != nullptr)
        nLen = strlen(pStrPwd);
    ...
    if( m_bCachedResourceZip ) {
        ...
        m_hResourceZip = (HANDLE)OpenZip((void*)sFile.GetData(), 0, 2, CPaintManagerUI::GetResourceZipPassword());
    }
}
...
CSkinManager* CPaintManagerUI::GetSkinManager()
{
    return CSkinManager::GetSkinManager();
}
...
SIZE CPaintManagerUI::GetInitSize()
{
    ...
    return GetScaleSize(m_szInitWindowSize, GetDPIObj()->GetScale());
}
...
void CPaintManagerUI::SetInitSize(int cx, int cy)
{
    ...
    if( m_pRoot == NULL && m_hWndPaint != NULL ) {
        ...
        int nScale = GetDPIObj()->GetScale();
        ::SetWindowPos(m_hWndPaint, NULL, 0, 0, GetScaleInt(cx, nScale), GetScaleInt(cy, nScale), SWP_NOZORDER | SWP_NOMOVE | SWP_NOACTIVATE);
    }
}

RECT CPaintManagerUI::GetSizeBox()
{
    return GetScaleRect(m_rcSizeBox, GetDPIObj()->GetScale());
}
...
{
    return GetScaleRect(m_rcCaption, GetDPIObj()->GetScale());
}
...
{
    return GetScaleSize(m_szRoundCorner, GetDPIObj()->GetScale());
}
...
{
    return GetScaleSize(m_szMinWindow, GetDPIObj()->GetScale());
}
...
{
    return GetScaleSize(m_szMaxWindow, GetDPIObj()->GetScale());
}
...
void CPaintManagerUI::SetOpacity(BYTE nOpacity)
{
	...
	if( m_hWndPaint != NULL ) 
    {
        ...
        Dui_SetLayeredWindowAttributes(m_hWndPaint, 0, nOpacity, LWA_ALPHA);
    ...
    case WM_PAINT:
    {
        ...
        if (m_bUpdateNeeded) {
            ...
            if (!::IsRectEmpty(&rcClient)) {
                ...
                m_rcLayeredUpdate = rcClient;  //20230220 orbit
        ...
        if (m_bLayered)
        {
            // 过早地设置 WS_EX_LAYERED，将导致收不到第一次 WM_PAINT 消息，所以放在这里设置分层属性
            DWORD dwExStyle = GetWindowLong(m_hWndPaint, GWL_EXSTYLE);
            ...
            if (m_hbmpOffscreen == NULL)
            {
                ...
                m_hbmpOffscreen = CRenderEngine::CreateBitmap(m_hDcPaint, nClientWidth, nClientHeight, &m_pOffscreenBits);
                //m_hbmpOffscreen = CRenderEngine::CreateARGB32Bitmap(m_hDcPaint, nClientWidth, nClientHeight, (COLORREF**)&m_pOffscreenBits);
            ...
            {
                ZeroMemory((unsigned int*)m_pOffscreenBits + y * nClientWidth + rcPaint.left, (rcPaint.right - rcPaint.left) * 4);
            }
            
            //DUI__Trace(_T("UIMManager::m_pRoot->Paint() Using Offscreen DC: %08x\n"), m_hDcOffscreen);
...

            {
                for (int i = 0; i < m_aNativeWindow.GetSize(); ) {
                    ...
                    //DUI__Trace(_T("UIMManager::layer_Paint() Using Offscreen DC: %08x\n"), hChildMemDC);
            ...
            */
            Dui_UpdateLayeredWindow(m_hWndPaint, NULL, &pt, &szWindow, m_hDcOffscreen,
                &ptSrc, 0, &blendPixelFunction, ULW_ALPHA);
        ...
        else
        {
            ...
            HBITMAP hOldBitmap = (HBITMAP) ::SelectObject(m_hDcOffscreen, m_hbmpOffscreen);
            int iSaveDC = ::SaveDC(m_hDcOffscreen);
            m_pRoot->Paint(m_hDcOffscreen, ps.rcPaint, NULL);
            {
                for (int i = 0; i < m_aNativeWindow.GetSize(); ) {
                    HWND hChildWnd = static_cast<HWND>(m_aNativeWindow[i]);
                    if (!::IsWindow(hChildWnd)) {
                    ...
                    ::ZeroMemory(pChildBitmapBits, (rcChildWnd.right - rcChildWnd.left) * (rcChildWnd.bottom - rcChildWnd.top) * 4);
                    HBITMAP hOldChildBitmap = (HBITMAP) ::SelectObject(hChildMemDC, hChildBitmap);
                    //DUI__Trace(_T("UIMManager::nonlayer_Paint() Using Offscreen DC: %08x\n"), hChildMemDC);
                    ::SendMessage(hChildWnd, WM_PRINT, (WPARAM)hChildMemDC, (LPARAM)(PRF_CHECKVISIBLE | PRF_CHILDREN | PRF_CLIENT | PRF_OWNED));
                    COLORREF* pChildBitmapBit;
                    for (LONG y = 0; y < rcChildWnd.bottom - rcChildWnd.top; y++) {
                        for (LONG x = 0; x < rcChildWnd.right - rcChildWnd.left; x++) {
...
BOOL CPaintManagerUI::Receive(SkinChangedParam param)
{
    //CControlUI* pRoot = GetRoot();
    CControlUI* pRoot = FindControl(_T("wnd_frame"));
    if (pRoot != NULL) {
        if (param.type & SKIN_NOTIFY_BKCOLOR) {
            pRoot->SetBkColor(param.bkcolor);
            pRoot->SetBkImage(_T(""));
        }
        if(param.type & SKIN_NOTIFY_BKIMAGE) {
            pRoot->SetBkColor(0x00000000);
            pRoot->SetBkImage(param.bgimage);
        }
        if(param.type & SKIN_NOTIFY_OPACITY) {
            m_nOpacity = param.opacity;
        }
...
void CPaintManagerUI::SetPaintWindowBackground(LPCTSTR bkImage, DWORD bkColor, int opacity)
{
    CControlUI* pRoot = FindControl(_T("wnd_frame"));
    if (pRoot != NULL) {
        if (_tcslen(bkImage) != 0) {
            pRoot->SetBkColor(0x00000000);
            pRoot->SetBkImage(bkImage);
        }
        else {
            if (bkColor != 0) {
                pRoot->SetBkColor(bkColor);
                pRoot->SetBkImage(_T(""));
            }
        }
        m_nOpacity = opacity;
...
UINT_PTR CPaintManagerUI::MessageLoop()
{
    MSG msg = { 0 };
...
void CPaintManagerUI::AdjustWindowByDPI(int iDPI)
{
    int scale1 = GetDPIObj()->GetScale();
    GetDPIObj()->SetDPI(iDPI);
    int scale2 = GetDPIObj()->GetScale();
    if (scale1 != scale2)
    {
        ResetDPIResource(iDPI);
    }
    RECT rcWnd = { 0 };
...
void CPaintManagerUI::SetAllDPI(int iDPI)
{
    for (int i = 0; i < m_aPreMessages.GetSize(); i++) {
        CPaintManagerUI* pManager = static_cast<CPaintManagerUI*>(m_aPreMessages[i]);
        pManager->AdjustWindowByDPI(iDPI);
    }
}

void CPaintManagerUI::ResetDPIResource(int nDPI)
{
    GetDPIObj()->SetDPI(nDPI);
    RemoveAllImages();
    ...
    CDuiPtrArray *richEditList = FindSubControlsByClass(GetRoot(), DUI_CTR_RICHEDIT);
...
void CPaintManagerUI::RebuildFont(TFontInfo * pFontInfo)
{
    ...
    lf.lfCharSet = DEFAULT_CHARSET;
    lf.lfHeight = -GetScaleInt(pFontInfo->iSize, GetDPIObj()->GetScale());
...
#ifdef _DEBUG
BOOL CheckTimerIdConflict(CDuiPtrArray& aTimers, UINT nTimerID)
{
    for (int i = 0; i < aTimers.GetSize(); i++) {
        TIMERINFO* pTimer = static_cast<TIMERINFO*>(aTimers[i]);
        if ((pTimer->nLocalID == nTimerID) && !pTimer->bKilled)
            return FALSE;
    }
...
#ifdef _DEBUG
    //ASSERT(CheckTimerIdConflict(m_aTimers, nTimerID));
#endif
...
void CPaintManagerUI::SendNotify(UINT uId, LPCTSTR pstrName, LPCTSTR pstrVirtualWnd, LPCTSTR pstrMessage)
{
    TNotifyUI Msg;
    Msg.sName = pstrName;
    Msg.uId = uId;
    if(m_bUsedVirtualWnd)
        Msg.sVirtualWnd = pstrVirtualWnd;
    Msg.sType = pstrMessage;
    Msg.pSender = NULL;
    Msg.wParam = NULL;
    Msg.lParam = NULL;
    SendNotify(Msg, true, true); 
}
...
{
    TNotifyUI Msg;
    Msg.sName = pControl->GetName();
    Msg.uId = pControl->GetID();
...
void CPaintManagerUI::SendNotify(TNotifyUI& Msg, bool bAsync /*= false*/, bool bEnableRepeat /*= true*/)
{
    ...
    //20230327 菜单通知时因为消息投递过程中菜单已经销毁，所以 Msg.pSender 可能是 NULL
    //在专为菜单消息设计的 SendNotify 中处理 VirtualWnd
...
void CPaintManagerUI::SetDefaultFont(LPCTSTR pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic, bool bShared)
{
    ...
    lf.lfCharSet = DEFAULT_CHARSET;
    //lf.lfHeight = -nSize;
    lf.lfHeight = -GetScaleInt(nSize, GetDPIObj()->GetScale());
...
const TImageInfo* CPaintManagerUI::AddImage(LPCTSTR bitmap, HICON hIcon, int iWidth, int iHeight, bool bAlpha, bool bShared)
{
    if( hIcon == NULL || iWidth <= 0 || iHeight <= 0 ) return NULL;
...
void CPaintManagerUI::SetWindowAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    if( _tcsicmp(pstrName, _T("size")) == 0 ) {
        ...
        SetInitSize(cx, cy);
        //SetInitSize(GetScaleInt(cx, GetDPIObj()->GetScale()), GetScaleInt(cy, GetDPIObj()->GetScale()));
    } 
        ...
        SetUseGdiplusText(_tcscmp(pstrValue, _T("true")) == 0);
    }
...
void CPaintManagerUI::TranslateLanguage(CDuiString& strText)
{
    m_SharedResInfo.m_LangTrans.TranslateLanguage(strText);
}

LPCTSTR CPaintManagerUI::TranslateLanguage(LPCTSTR lpText)
{
    LPCTSTR lpTrans = m_SharedResInfo.m_LangTrans.TranslateLanguage(lpText);
    return (lpTrans != NULL) ? lpTrans : lpText;
}

LPCTSTR CPaintManagerUI::TranslateErrorCode(UINT code)
{
    static CDuiString strTmp;
    strTmp.Format(_T("%d"), code);
    LPCTSTR lpTrans = m_SharedResInfo.m_LangTrans.TranslateErrorCode(strTmp);
    return (lpTrans != NULL) ? lpTrans : (LPCTSTR)strTmp;
}

bool CPaintManagerUI::LoadLanguage(LPCTSTR lanfile, bool bUpdate)
{
    m_SharedResInfo.m_LangTrans.Reset();
    if (!m_SharedResInfo.m_LangTrans.LoadLanguage(lanfile))
        return false;
...
CDuiString CPaintManagerUI::GetLanguageName()
{
    return m_SharedResInfo.m_LangTrans.GeLanguagetName();
}
...
CControlUI* CPaintManagerUI::FindControl(UINT uID) const
{
    ASSERT(m_pRoot);
    return m_pRoot->FindControl(__FindControlFromID, &uID, 0);
}
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlFromID(CControlUI* pThis, LPVOID pData)
{
    UINT* pID = static_cast<UINT *>(pData);
    return (pThis->GetID() == *pID) ? pThis : NULL;
}
