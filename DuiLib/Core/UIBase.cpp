		...
		for( int i = 0; i< m_VirtualWndMap.GetSize(); i++ ) {
			if( LPCTSTR key = m_VirtualWndMap.GetAt(i) ) {
				if( _tcsicmp(key, msg.sVirtualWnd.GetData()) == 0 ){
					...
					if( pObject && pObject->_HandleNotify(msg) )
						return;
				}
			}
		}
...
BOOL CWindowWnd::SetWindowPos(HWND hWndInsertAfter, int X, int Y, int cx, int cy, UINT uFlags)
{
    ASSERT(::IsWindow(m_hWnd));
