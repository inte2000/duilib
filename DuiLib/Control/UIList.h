...
public:
    ...
    //20230210 增加一组动态操作接口
    BOOL InsertColumn(int nCol, LPCTSTR lpszColumnHeaderStr, UINT uTextStyle = DT_LEFT, int nWidth = -1);
    ...
    void SetItemContent(int nItem, int nColumn, LPCTSTR Text, LPCTSTR Name);
    void SetItemContent(int nItem, int nColumn, LPCTSTR lpImage, int nWidth, int nHeight);
    void SetItemContent(int nItem, int nColumn, CControlUI* pControl);
    CControlUI* GetItemContent(int nItem, int nColumn);
    void SetItemUserData(int nItem, LPCTSTR lpUserData);
    const CDuiString& GetItemUserData(int nItem);
    void SetItemTag(int nItem, UINT_PTR tag);
...
{
    DECLARE_DUICONTROL(CListHBoxElementUI)
public:
    CListHBoxElementUI();
