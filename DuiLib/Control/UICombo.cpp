...
void CComboWnd::Init(CComboUI* pOwner)
{
    ...
    // orbit : 解决combox 列表窗口弹出后窗口deactive的问题
    //::SetWindowPos(m_hWnd, HWND_TOP, 0, 0, 0, 0, SWP_SHOWWINDOW | SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE);
...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
        ...
        if (m_pOwner->GetManager()->IsLayered())
            m_pm.SetLayeredOpacity(m_pOwner->GetManager()->GetLayeredOpacity());
...
int CComboUI::AddString(LPCTSTR lpText, LPCTSTR lpUserData)
{
    CListLabelElementUI* pItem = new CListLabelElementUI;
    pItem->SetText(lpText);
    pItem->SetUserData(lpUserData);
    //pItem->SetFixedHeight(m_ListInfo.uFixedHeight);
    //pListItem->SetTextColor(m_ListInfo.dwTextColor);
    if (CComboUI::Add(pItem))
    {
        return GetCount() - 1;
    }
...
int CComboUI::DeleteString(int iIndex)
{
    CComboUI::RemoveAt(iIndex, false);

    return GetCount();
}

int CComboUI::FindString(int nStartAfter, LPCTSTR lpText) const
{
    int starti = (nStartAfter == -1) ? 0 : nStartAfter + 1;
    while (starti < GetCount())
    {
        CControlUI* p = GetItemAt(starti);
        if (p != NULL) {
            if (p->GetText().Compare(lpText) == 0)
                return starti;
        }
...
CDuiString CComboUI::GetItemUserData(int iIndex)
{
    static CDuiString empData;
...
bool CComboUI::GetItemUserData(int iIndex, CDuiString& text)
{
    CControlUI* pItem = GetItemAt(iIndex);
    if (pItem != NULL)
    {
        text = pItem->GetUserData();
        return true;
    }
...
bool CComboUI::SetItemUserData(int iIndex, LPCTSTR lpText)
{
    CControlUI* pItem = GetItemAt(iIndex);
    if (pItem != NULL)
    {
        pItem->SetUserData(lpText);
        return true;
    }
...
{
    if (m_cxyFixed.cy == 0) {
        SIZE rtnSize = { m_cxyFixed.cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + 4 }; //+8 20220419 orbit
        return GetScaleSize(rtnSize, m_nScale);
    }
...
{
    //return m_szDropBox;
    return GetScaleSize(m_szDropBox, m_nScale); //orbit 20220419
}
...
{
    //DUI__Trace(_T("CComboUI::SetAttribute(%s, %s)\n"), pstrName, pstrValue);
