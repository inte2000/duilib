...
CRichEditUI::CRichEditUI() : m_pTwh(NULL), m_bVScrollBarFixing(false), m_bWantTab(true), m_bWantReturn(true), 
    m_bWantCtrlReturn(true), m_bTransparent(true), m_bRich(true), m_bReadOnly(false), m_bWordWrap(false), m_dwTextColor(0xFF000001), m_iFont(-1), 
	m_iLimitText(cInitTextMax), m_lTwhStyle(ES_MULTILINE), m_bDrawCaret(true), m_bInited(false), m_chLeadByte(0), m_dwTipValueColor(0xFFBAC0C5)
{
#ifndef _UNICODE
...
void CRichEditUI::DoInit()
{
    ...
    cs.lpszName = CPaintManagerUI::TranslateLanguage(m_sText.GetData());
...
bool CRichEditUI::DoPaint(HDC hDC, const RECT& rcPaint, CControlUI* pStopControl)
{
    ...
    if (GetTextLength() == 0 && !sTipValue.IsEmpty())
    {
        RECT rc;
        m_pTwh->GetControlRect(&rc);
        CRenderEngine::DrawText(hDC, m_pManager, rc, sTipValue, m_dwTipValueColor, m_iFont, DT_VCENTER);
    }
