...
{
    RECT rcInset = GetScaleRect(m_rcInset, m_nScale);

	if( m_pHeader != NULL ) { // 设置header各子元素x坐标,因为有些listitem的setpos需要用到(临时修复)
		int iLeft = rc.left + rcInset.left;
		int iRight = rc.right - rcInset.right;
    ...
    if( event.Type == UIEVENT_KEYDOWN )
    {
        if (IsKeyboardEnabled() && IsEnabled()) {
            ...
            case VK_UP:
                ...
                break; //orbit 解决键盘控制选择问题（20240325 确认官方库做了同样的修改）
            case VK_DOWN:
                SelectItem(FindSelectable(m_iCurSel + 1, true), true);
                break; //orbit 解决键盘控制选择问题
            case VK_PRIOR:
                PageUp();
                break; //orbit 解决键盘控制选择问题
            case VK_NEXT:
...
{
    return GetScaleRect(m_ListInfo.rcTextPadding, m_nScale);
}
...
void CListUI::SetItemBkImage(LPCTSTR pStrImage)
{
    ...
    ParseDrawInfo(m_pManager, m_ListInfo.diBk, m_nScale);
	Invalidate();
}
...
void CListUI::SetSelectedItemImage(LPCTSTR pStrImage)
{
    ...
    ParseDrawInfo(m_pManager, m_ListInfo.diSelected, m_nScale);
	Invalidate();
}
...
void CListUI::SetHotItemImage(LPCTSTR pStrImage)
{
    ...
    ParseDrawInfo(m_pManager, m_ListInfo.diHot, m_nScale);
	Invalidate();
}
...
void CListUI::SetDisabledItemImage(LPCTSTR pStrImage)
{
    ...
    ParseDrawInfo(m_pManager, m_ListInfo.diDisabled, m_nScale);
	Invalidate();
}
...
BOOL CListUI::InsertColumn(int nCol, LPCTSTR lpszColumnHeaderStr, UINT uTextStyle, int nWidth)
{
    ...
    pHeaderItem->SetAttribute(_T("sepimage"), _T("control\\list_header_sep.png"));
    pHeaderItem->SetAttribute(_T("sepwidth"), _T("1"));
    pHeaderItem->SetAttribute(_T("pushedimage"), _T("control\\list_header_pushed.png"));
    pHeaderItem->SetAttribute(_T("hotimage"), _T("control\\list_header_hot.png"));
    pHeaderItem->SetAttribute(_T("normalimage"), _T("control\\list_header_bg.png"));
...
void CListUI::SetItemContent(int nItem, int nColumn, LPCTSTR Text, LPCTSTR Name)
{
    CLabelUI* pLabel = new CLabelUI;
    pLabel->SetText(Text);//控件属性就根据需求设置吧,我简单设置一下
    pLabel->SetTextStyle(m_ListInfo.uTextStyle);
    if(m_ListInfo.dwTextColor != 0)
        pLabel->SetTextColor(m_ListInfo.dwTextColor);
    if(m_ListInfo.nFont >= 0)
        pLabel->SetFont(m_ListInfo.nFont);
    pLabel->SetAttribute(_T("endellipsis"), _T("true"));
    pLabel->SetName(Name);
    SetItemContent(nItem, nColumn, pLabel);//添加到父控件
}

void CListUI::SetItemContent(int nItem, int nColumn, LPCTSTR lpImage, int nWidth, int nHeight)
{
    CLabelUI* pLabel = new CLabelUI;
    pLabel->SetManager(GetManager(), NULL, false);
    pLabel->SetBorderSize(0);
    pLabel->SetBkImage(lpImage);
    pLabel->SetFixedWidth(nWidth);
    pLabel->SetFixedHeight(nHeight);
    SetItemContent(nItem, nColumn, pLabel);//添加到父控件
}

void CListUI::SetItemContent(int nItem, int nColumn, CControlUI* pControl)
{
    CHorizontalLayoutUI* pSubHor = GetListSubItem(nItem, nColumn);
    pSubHor->SetAttribute(_T("inset"), _T("3,1,3,1"));
    pSubHor->SetChildVAlign(DT_VCENTER);
    pSubHor->Add(pControl);//添加到父控件
}

CControlUI* CListUI::GetItemContent(int nItem, int nColumn)
{
    CHorizontalLayoutUI* pSubHor = GetListSubItem(nItem, nColumn);
    if (pSubHor && (pSubHor->GetCount() > 0))
    {
        return pSubHor->GetItemAt(0);
    }

    return NULL;
}

void CListUI::SetItemUserData(int nItem, LPCTSTR lpUserData)
{
    CControlUI* pItem = GetItemAt(nItem);
    if (pItem != NULL)
        pItem->SetUserData(lpUserData);
}

const CDuiString& CListUI::GetItemUserData(int nItem)
{
    static CDuiString emptyData;
...
void CListUI::SetItemTag(int nItem, UINT_PTR tag)
{
    CControlUI* pItem = GetItemAt(nItem);
    if (pItem != NULL)
        pItem->SetTag(tag);
...
UINT_PTR CListUI::GetItemTag(int nItem)
{
    CControlUI* pItem = GetItemAt(nItem);
    if (pItem != NULL)
    {
...
CListContainerElementUI* CListUI::GetListItem(int iIndex)
{
...
bool CListBodyUI::SortItems(PULVCompareFunc pfnCompare, UINT_PTR dwData, int& iCurSel)
{
	...
	for (int i = 0; i < m_items.GetSize(); ++i)
	{
        ...
        //Orbit20240325 bugfix: list调用sort后选中错乱（未正确更新索引） https://github.com/duilib/duilib/pull/182
...
{
    SIZE cXY = {0, GetScaleInt(m_cxyFixed.cy, m_nScale) };
	if( cXY.cy == 0 && m_pManager != NULL ) {
		for( int it = 0; it < m_items.GetSize(); it++ ) {
			cXY.cy = MAX(cXY.cy,static_cast<CControlUI*>(m_items[it])->EstimateSize(szAvailable).cy);
		}
		int nMin = m_pManager->GetDefaultFontInfo()->tm.tmHeight + 4; //+8 20220419 orbit
...
{
	return GetScaleInt(m_iSepWidth, m_nScale);
}
...
void CListHeaderItemUI::SetSepImage(LPCTSTR pStrImage)
{
    ...
    ParseDrawInfo(m_pManager, m_diSep, m_nScale);
	Invalidate();
}
...
void CListHeaderItemUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
    {
        ...
        // orbit 调整大小，用户体验比较好一点
        if (rcSeparator.left >= 2)
            rcSeparator.left -= 2;
        rcSeparator.right += 2;
    ...
    if( event.Type == UIEVENT_MOUSEMOVE )
    {
        if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
            ...
            if( rc.right - rc.left > GetMinWidth() ) {
                ...
                m_cxyFixed.cx = MulDiv(rc.right - rc.left, 100, m_nScale);
                //m_bFixedWidth = true;
                //m_rcItem.right = rc.right;
                m_cXY.cx = rc.right - rc.left;
                ptLastMouse = event.ptMouse;
                if( GetParent() ) 
                    GetParent()->NeedParentUpdate();
            }
...
{
    if (m_cxyFixed.cy == 0) {
        return CDuiSize(GetScaleInt(m_cxyFixed.cx, m_nScale), m_pManager->GetDefaultFontInfo()->tm.tmHeight + 4); //+8 20220419 orbit
    }
...
{
    int iSepWidth = GetScaleInt(m_iSepWidth, m_nScale);
    if(iSepWidth >= 0 ) return CDuiRect(m_rcItem.right - iSepWidth, m_rcItem.top, m_rcItem.right, m_rcItem.bottom);
    else return CDuiRect(m_rcItem.left, m_rcItem.top, m_rcItem.left - iSepWidth, m_rcItem.bottom);
}
...
void CListHeaderItemUI::PaintStatusImage(HDC hDC)
{
    ...
    if (m_iSepWidth > 0) {
        ...
        if( !DrawImage(hDC, m_diSep) ) {
            if (m_dwSepColor != 0) {
                int iSepWidth = GetScaleInt(m_iSepWidth, m_nScale);
                RECT rcSepLine = { rcThumb.left + iSepWidth /2, rcThumb.top, rcThumb.left + iSepWidth /2, rcThumb.bottom};
                CRenderEngine::DrawLine(hDC, rcSepLine, iSepWidth, GetAdjustColor(m_dwSepColor));
            }
        }
    }
}
...
bool CListElementUI::Select(bool bSelect, bool bTriggerEvent)
{
    ...
    //Orbit20240325 https://github.com/duilib/duilib/pull/182
...
SIZE CListLabelElementUI::EstimateSize(SIZE szAvailable)
{
    ...
    if (m_cxyFixed.cx > 0) {
        if (m_cxyFixed.cy > 0) return GetScaleSize(m_cxyFixed, m_nScale); // m_cxyFixed; orbit 20220419
        else if (pInfo->uFixedHeight > 0) return CDuiSize(GetScaleInt(m_cxyFixed.cx, m_nScale), GetScaleInt(pInfo->uFixedHeight, m_nScale));// CDuiSize(m_cxyFixed.cx, pInfo->uFixedHeight);
    }
    ...
    if (m_bNeedEstimateSize) {
        ...
        if ((pInfo->uTextStyle & DT_SINGLELINE) != 0) {
            if( m_cxyFixedLast.cy == 0 ) {
                m_cxyFixedLast.cy = m_pManager->GetFontInfo(pInfo->nFont)->tm.tmHeight + MulDiv(4, 100, m_nScale); //+8 20220419 orbit
            ...
            if (m_cxyFixedLast.cx == 0) {
                ...
                if( pInfo->bShowHtml ) {
                    int nLinks = 0;
                    CRenderEngine::DrawHtmlText(m_pManager->GetPaintDC(), m_pManager, rcText, sText, 0, NULL, NULL, nLinks, pInfo->nFont, DT_CALCRECT | pInfo->uTextStyle & ~DT_RIGHT & ~DT_CENTER);
                }
                else {
                    CRenderEngine::DrawText(m_pManager->GetPaintDC(), m_pManager, rcText, sText, 0, pInfo->nFont, DT_CALCRECT | pInfo->uTextStyle & ~DT_RIGHT & ~DT_CENTER);
                }
    ...
    //return m_cxyFixedLast;   20220419 orbit 
...
LPCTSTR CListTextElementUI::GetText(int iIndex) const
{
    ...
    if (pText)
    {
        ...
        return CPaintManagerUI::TranslateLanguage(pText->GetData());
    }
    return NULL;
}
...
SIZE CListTextElementUI::EstimateSize(SIZE szAvailable)
{
    ...
    else strText = CControlUI::GetText();
...
{
    // orbit 20230326
    //手工添加的 Control 无法直接使用 list 属性中的颜色，所以这里要给所有的子控件设置一下颜色
    if (m_pOwner != NULL) {
        TListInfoUI* pInfo = m_pOwner->GetListInfo();
        if (pInfo != NULL) {
            DWORD iTextColor = pInfo->dwTextColor;
            ...
            //一般来说，一个 CListContainerElementUI 只有一个子控件，但是也不排除用户添加多个的情况
            for (int i = 0; i < CContainerUI::GetCount(); i++) {
                CControlUI *pItem = CContainerUI::GetItemAt(i);
                if (pItem != NULL) {
                    CLabelUI* pLabel = static_cast<CLabelUI*>(pItem->GetInterface(DUI_CTR_LABEL));
                    if(pLabel != NULL)
...
SIZE CListContainerElementUI::EstimateSize(SIZE szAvailable)
{
    ...
    //SIZE cXY = m_cxyFixed;
    SIZE cXY = GetScaleSize(m_cxyFixed, m_nScale);

    if( cXY.cy == 0 ) {
        //cXY.cy = pInfo->uFixedHeight;
        cXY.cy = GetScaleInt(pInfo->uFixedHeight, m_nScale);
    }
...
void CListContainerElementUI::SetPos(RECT rc, bool bNeedInvalidate)
{
    ...
    for (int i = 0; i < nCount; i++)
    {
        ...
        {  
            ...
            //if (pInfo->uTextStyle == DT_CENTER)
            //{
            //    int wgap = (pInfo->rcColumn[i].right - pInfo->rcColumn[i].left) - (rt.right - rt.left);
            //    rt.left += wgap;
            //}
            pControl->SetPos(rt);
        }  
...
//
//
IMPLEMENT_DUICONTROL(CListHBoxElementUI)
