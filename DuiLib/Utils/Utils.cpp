...
namespace DuiLib
{
	...
	std::vector<CDuiString> SplitString(const CDuiString& strMultiLine, LPCTSTR lpMark)
	{
		std::vector<CDuiString> strArray;

		CDuiString strLine = strMultiLine;
		int length = strMultiLine.GetLength();
		int spLen = (int)_tcslen(lpMark);
		int idx;
		CDuiString strTmp;
		while ((idx = strLine.Find(lpMark, 0)) != -1)
		{
			strTmp = strLine.Left(idx);
			strArray.push_back(strTmp);
			strLine = strLine.Right(length - idx - spLen);
			length = strLine.GetLength();
		}
		...
		while( *ppItem ) {
			if( (*ppItem)->Key == key ) {
				...
				*ppItem = (*ppItem)->pNext;
				...
				if (pKill->pPrev) //Orbit20240327 同步官方库 pr
